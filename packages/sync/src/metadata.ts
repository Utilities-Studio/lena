...
export const syncChangeMetadataSchema = z
	...
	.superRefine((change, context) => {
		if (
			...
			change.baseRevision >= change.revision
		) {
			context.addIssue({
				code: 'custom',
...
export function parseSyncChangeMetadata(
	input: unknown
): Result<SyncChangeMetadata, LenaError> {
...
export function parseSyncCheckpoint(
	input: unknown
): Result<SyncCheckpoint, LenaError> {
...
export function parseSyncTombstone(
	input: unknown
): Result<SyncTombstone, LenaError> {
