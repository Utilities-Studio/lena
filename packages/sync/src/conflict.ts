...
function compareVersions<Value>(
	...
	const timestampOrder = compareIsoTimestamps(
		left.metadata.changedAt,
		right.metadata.changedAt
	)
	if (timestampOrder !== 0) return timestampOrder
	const replicaOrder = left.metadata.replicaId.localeCompare(
		right.metadata.replicaId
	)
...
export function resolveSyncConflict<Value>(
	...
	const [first, second] =
		compareVersions(left, right) <= 0 ? [left, right] : [right, left]
