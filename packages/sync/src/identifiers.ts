...
export const syncConsentRequestIdSchema =
	syncIdSchema.brand<'SyncConsentRequestId'>()
...
export function parseSyncChangeId(
	value: unknown
): Result<SyncChangeId, LenaError> {
	return parseSyncId(value, 'sync_change_id', syncChangeIdSchema)
}

export function parseSyncCheckpointId(
	value: unknown
): Result<SyncCheckpointId, LenaError> {
	return parseSyncId(value, 'sync_checkpoint_id', syncCheckpointIdSchema)
}

export function parseSyncConsentId(
	value: unknown
): Result<SyncConsentId, LenaError> {
	return parseSyncId(value, 'sync_consent_id', syncConsentIdSchema)
}

export function parseSyncConsentRequestId(
	value: unknown
): Result<SyncConsentRequestId, LenaError> {
	return parseSyncId(
		value,
		'sync_consent_request_id',
		syncConsentRequestIdSchema
	)
}

export function parseSyncReplicaId(
	value: unknown
): Result<SyncReplicaId, LenaError> {
	return parseSyncId(value, 'sync_replica_id', syncReplicaIdSchema)
}

export function parseSyncTombstoneId(
	value: unknown
): Result<SyncTombstoneId, LenaError> {
	return parseSyncId(value, 'sync_tombstone_id', syncTombstoneIdSchema)
}

export function parseSyncProtocolVersion(
	value: unknown
): Result<SyncProtocolVersion, LenaError> {
	const parsed = syncProtocolVersionSchema.safeParse(value)
	if (!parsed.success) {
		return err(
			new LenaError('invalid_input', { boundary: 'sync_protocol_version' })
		)
	}
