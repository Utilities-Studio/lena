...
const syncConsentEventSchema = z.discriminatedUnion('type', [
	z.strictObject({
		request: hostedSyncConsentRequestSchema,
...
export type UnrelatedSyncAuthoritySource = z.infer<
	typeof unrelatedSyncAuthoritySourceSchema
>
	...
	return match(parsedEvent.data)
		...
		.with({ type: 'request' }, (event) => {
			...
			if (
				...
				state.latestRequest.requestId === request.requestId
			) {
				return state.latestRequest.disclosureVersion ===
			...
			if (
				...
				compareIsoTimestamps(
					request.requestedAt,
					state.latestRequest.requestedAt
				) <= 0
			) {
				return err(
					new LenaError('conflict', {
		...
		.with({ type: 'grant' }, (event) => {
			if (
				...
				compareIsoTimestamps(event.grant.grantedAt, state.request.requestedAt) <
					0
			) {
				return err(
					new LenaError('invalid_state_transition', {
		...
		.with({ type: 'withdraw' }, (event) => {
			if (
				...
				state.consent.consentId !== event.consentId
			) {
				return err(
					new LenaError('invalid_state_transition', {
						boundary: 'sync_consent'
			...
			if (
				compareIsoTimestamps(event.withdrawnAt, state.consent.grantedAt) < 0
			) {
				return err(
					new LenaError('invalid_state_transition', {
						boundary: 'sync_consent'
