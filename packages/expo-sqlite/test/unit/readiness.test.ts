...
describe("Expo SQLite readiness", () => {
  ...
  test("rejects Expo Go, missing backup, and missing SQLCipher", () => {
    expect(
      assessExpoSqliteReadiness({ ...COMPLETE, expoGo: true }).contractRequirementsSatisfied,
    ).toBe(false);
    expect(
    ...
    expect(
      assessExpoSqliteReadiness({ ...COMPLETE, sqlCipher: false }).contractRequirementsSatisfied,
    ).toBe(false);
  });

  test("keeps derived search optional for canonical vault access", () => {
    const readiness = assessExpoSqliteReadiness({ ...COMPLETE, fts5: false, wal: false });
    expect(readiness.contractRequirementsSatisfied).toBe(true);
