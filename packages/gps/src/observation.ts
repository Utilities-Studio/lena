...
export const gpsAccuracyPolicySchema = z
	...
	.refine(
		({ preciseMaximumMeters, usableMaximumMeters }) =>
			usableMaximumMeters >= preciseMaximumMeters,
...
export function parseGpsAccuracyPolicy(
	input: unknown
): Result<GpsAccuracyPolicy, LenaError> {
	const parsed = gpsAccuracyPolicySchema.safeParse(input)
	if (!parsed.success) {
		return err(
			new LenaError('invalid_input', { boundary: 'gps_accuracy_policy' })
		)
	}
...
export function createPersistableCountryObservation(
	...
	if (
		input.sample.horizontalAccuracyMeters >
		input.accuracyPolicy.usableMaximumMeters
	) {
		return ok(null)
	}
	...
	const accuracyClass =
		input.sample.horizontalAccuracyMeters <=
		input.accuracyPolicy.preciseMaximumMeters
			? 'precise'
			: 'coarse'
	const confidence =
		accuracyClass === 'precise' && input.resolution.match === 'unambiguous'
			? 'high'
			: 'low'
