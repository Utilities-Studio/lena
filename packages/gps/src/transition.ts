...
export function parseGpsTransitionPolicy(input: unknown): Result<GpsTransitionPolicy, LenaError> {
...
function compareObservationOrder(left: GpsObservationOrder, right: GpsObservationOrder): number {
...
function compareManualCorrections(left: GpsManualCorrection, right: GpsManualCorrection): number {
  const timestampOrder = compareIsoTimestamps(left.correctedAt, right.correctedAt);
  return timestampOrder === 0
...
function reduceGpsObservation(
  ...
  if (
    state.lastManualCorrection !== null &&
    compareIsoTimestamps(observation.observedAt, state.lastManualCorrection.correctedAt) <= 0
  ) {
    return Object.freeze({ effects: Object.freeze([]), state });
  }
  if (
...
function eventTimestamp(event: GpsTransitionEvent): IsoTimestamp {
  return match(event)
    ...
    .with({ type: "manual_correction" }, ({ correction }) => correction.correctedAt)
    .exhaustive();
}

function eventIdentifier(event: GpsTransitionEvent): string {
  return match(event)
    .with({ type: "observation" }, ({ observation }) => observation.observationId)
    .with({ type: "manual_correction" }, ({ correction }) => correction.correctionId)
    .exhaustive();
}
