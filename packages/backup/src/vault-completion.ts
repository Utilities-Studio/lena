...
function assertCoverage(
	...
	if (
		...
		(target.commitSequence === snapshot.commitSequence &&
			target.mutationId !== snapshot.mutationId)
	) {
	...
	try {
		const completed = await database.transaction(
			async (transaction) => {
				...
				const existing = await transaction
					...
					.where(
						eq(
							lenaVerifiedBackupGenerationsTable.generationId,
		...
		return err(
			cause instanceof AbortBackupCompletion
				? cause.lenaError
				: new LenaError('internal', {
						boundary: 'backup_completion_transaction'
					})
		)
	}
}
