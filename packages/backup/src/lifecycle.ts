...
const backupObjectByteLengthSchema = z.number().int().refine(Number.isSafeInteger).min(0);
...
export type BackupAttempt =
  ...
  | (BackupAttemptBase & {
      ...
      readonly verification: VerifiedGeneration | PersistedGenerationVerificationClaim;
    })
  | (BackupAttemptBase & {
...
export type BackupAttemptEvent =
  ...
  | Readonly<{ at: IsoTimestamp; provider: RemoteBackupProvider; type: "queue-upload" }>
  | Readonly<{
      at: IsoTimestamp;
      claimId: EffectId;
      type: "claim-upload";
...
export function isRuntimeClaimedBackupAttempt(
  ...
  return typeof value === "object" && value !== null && runtimeClaimedBackupAttempts.has(value);
}
...
function validRemoteObjectPath(value: string): boolean {
  return value.trim().length > 0 && value.length <= 1_024 && !/[\r\n]/.test(value);
}
...
function getBackupResumeCheckpoint(attempt: BackupAttempt): BackupResumeCheckpoint | null {
  return match(attempt)
    ...
    .with({ state: P.union("upload-pending", "uploading", "remote-uploaded") }, (current) =>
      Object.freeze({
        objectByteLength: current.objectByteLength,
        objectChecksum: current.objectChecksum,
