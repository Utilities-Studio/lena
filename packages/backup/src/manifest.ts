...
export type BackupEncryptionDescriptor = z.infer<
	typeof backupEncryptionDescriptorSchema
>
...
export const backupRecordCountsSchema = z
	...
	.superRefine((recordCounts, context) => {
		if (Object.keys(recordCounts).length > MAX_RECORD_TYPE_COUNT) {
			context.addIssue({
				code: 'custom',
				message: 'record_type_limit_exceeded'
			})
		}
	})
	.readonly()
...
export const backupPayloadDescriptorSchema = z
	.strictObject({
		attachmentByteLength: boundedNonNegativeIntegerSchema(
...
export type BackupPayloadDescriptor = z.infer<
	typeof backupPayloadDescriptorSchema
>
...
export type BackupSnapshotWatermark = z.infer<
	typeof backupSnapshotWatermarkSchema
>
...
export function parseGenerationManifest(
	value: unknown
): Result<GenerationManifest, LenaError> {
	const version = manifestVersionSchema.safeParse(value)
	if (
		version.success &&
		version.data.formatVersion !== BACKUP_MANIFEST_FORMAT_VERSION
	) {
