...
export function isAuthorizedRetentionPlan(value: unknown): value is RetentionPlan {
  return (
    typeof value === "object" &&
    value !== null &&
    (value as { readonly [retentionPlanMarker]?: unknown })[retentionPlanMarker] === true &&
...
export function planGenerationRetention(
  ...
  const ordered = orderBy(
    generations,
    [(generation) => generation.completedAt, (generation) => generation.generationId],
    ["desc", "asc"],
  );
  ...
  const newestVerified = ordered.find((generation) => generation.verification !== null);
  ...
  const plan = Object.freeze({
    ...
    keep: Object.freeze(
      ordered.filter((item) => keep.has(item.generationId)).map((item) => item.generationId),
    ),
    vaultId,
  });
