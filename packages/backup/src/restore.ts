...
import { isRuntimeValidatedRestoreCandidate, type ValidatedRestoreCandidate } from "./discovery";
...
export type ActivePointerUnknownCode = z.infer<typeof activePointerUnknownCodeSchema>;

const restoreByteLengthSchema = z.number().int().refine(Number.isSafeInteger).min(0);
...
export type RestoreGenerationBinding = z.infer<typeof restoreGenerationBindingSchema>;
...
export type RestoreEvent =
  ...
  | Readonly<{ at: IsoTimestamp; type: "record-ready" }>
  ...
  | Readonly<{ at: IsoTimestamp; failureCode: RestoreFailureCode; type: "fail" }>;
...
export const restoreAttemptSchema = z
  .discriminatedUnion("state", [
    z.strictObject({ ...restoreAttemptBaseShape, state: z.literal("discovered") }),
    z.strictObject({ ...restoreAttemptBaseShape, state: z.literal("staging") }),
    z.strictObject({ ...restoreAttemptBaseShape, state: z.literal("decrypted") }),
    z.strictObject({ ...restoreAttemptBaseShape, state: z.literal("manifest-validated") }),
    z.strictObject({ ...stagedRestoreAttemptShape, state: z.literal("vault-built") }),
    z.strictObject({ ...stagedRestoreAttemptShape, state: z.literal("ready-to-activate") }),
    z.strictObject({ ...stagedRestoreAttemptShape, state: z.literal("activating") }),
    z.strictObject({ ...stagedRestoreAttemptShape, state: z.literal("activated") }),
    z.strictObject({
      ...stagedRestoreAttemptShape,
      state: z.literal("verified"),
      verifiedAt: isoTimestampSchema,
    }),
    z.strictObject({ ...stagedRestoreAttemptShape, state: z.literal("rolling-back") }),
    z.strictObject({ ...stagedRestoreAttemptShape, state: z.literal("rolled-back") }),
    z.strictObject({
      ...stagedRestoreAttemptShape,
  ...
  .superRefine((attempt, context) => {
    ...
    if (attempt.state === "verified" && attempt.verifiedAt !== attempt.updatedAt) {
      context.addIssue({
        code: "custom",
...
export function createRestoreAttempt(input: {
  ...
  if (!isRuntimeValidatedRestoreCandidate(input.candidate)) {
    return err(new LenaError("integrity_failed", "Restore candidate lacks runtime verification"));
  }
  return ok(
    Object.freeze({
...
function invalidRestoreTransition(
  ...
  return err(
    new LenaError("invalid_state_transition", "Invalid restore state transition", { event, state }),
  );
}

function bindingMatches(attempt: RestoreAttempt, binding: RestoreGenerationBinding): boolean {
  return (
...
function bindingMismatch(): Result<never, LenaError> {
  return err(new LenaError("integrity_failed", "Restore checkpoint does not match generation"));
}
...
export function reduceRestoreAttempt(
  ...
  if (compareIsoTimestamps(event.at, attempt.updatedAt) < 0) {
    return err(new LenaError("invalid_timestamp", "Restore event predates durable checkpoint"));
  }
  ...
  return match(event)
    ...
    .with({ type: "record-vault-built" }, (nextEvent) => {
      ...
      if (nextEvent.stagingInstanceId === attempt.previousInstanceId) {
        return err(
          new LenaError("conflict", "Restore staging instance must not replace the live instance"),
        );
      }
      return ok(
        Object.freeze({
    ...
    .with({ type: "record-activated" }, (nextEvent) => {
      ...
      if (nextEvent.activeInstanceId !== attempt.stagingInstanceId) {
        return err(new LenaError("conflict", "Active pointer does not target staging instance"));
      }
      return ok(
        Object.freeze({
          ...base,
    ...
    .with({ type: "record-post-open-verified" }, (nextEvent) => {
      ...
      if (!bindingMatches(attempt, nextEvent.binding)) return bindingMismatch();
      if (nextEvent.activeInstanceId !== attempt.stagingInstanceId) {
        return err(new LenaError("conflict", "Verified instance is not the active instance"));
      }
      return ok(
        Object.freeze({
          ...base,
    ...
    .with({ type: "record-active-pointer-unknown" }, (nextEvent) => {
      ...
      if (
        ...
        (attempt.state === "rolling-back" && nextEvent.failureCode !== "rollback-outcome-unknown")
      ) {
        return err(new LenaError("invalid_input", "Pointer uncertainty code does not match stage"));
      }
      return ok(
        Object.freeze({
          ...base,
    ...
    .with({ type: "record-rolled-back" }, (nextEvent) => {
      ...
      if (nextEvent.activeInstanceId !== attempt.previousInstanceId) {
        return err(new LenaError("conflict", "Rollback did not restore the previous pointer"));
      }
      return ok(
        Object.freeze({
          ...base,
    ...
    .with({ type: "fail" }, (nextEvent) => {
      if (!restoreFailureCodeSchema.safeParse(nextEvent.failureCode).success) {
        return err(new LenaError("invalid_input", "Invalid restore failure code"));
      }
      if (
        attempt.state === "activating" ||
        attempt.state === "activated" ||
        ...
        return err(
          new LenaError(
            ...
            "A restore that may have changed the active pointer must roll back",
            { state: attempt.state },
          ),
        );
      }
 ...
 */
export function parseRestoreAttempt(value: unknown): Result<RestoreAttempt, LenaError> {
  const parsed = restoreAttemptSchema.safeParse(value);
  if (!parsed.success) {
    return err(new LenaError("invalid_input", "Invalid persisted restore attempt"));
  }
