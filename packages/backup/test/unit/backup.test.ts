...
const OTHER_VAULT_UUID = '018f3f5a-1d2c-4abc-8def-1123456789ab'
const GENERATION_UUID = '018f3f5a-1d2c-4abc-8def-2123456789ab'
const SECOND_GENERATION_UUID = '018f3f5a-1d2c-4abc-8def-3123456789ab'
const THIRD_GENERATION_UUID = '018f3f5a-1d2c-4abc-8def-4123456789ab'
const EFFECT_UUID = '018f3f5a-1d2c-4abc-8def-5123456789ab'
const SECOND_EFFECT_UUID = '018f3f5a-1d2c-4abc-8def-6123456789ab'
const PREVIOUS_INSTANCE_UUID = '018f3f5a-1d2c-4abc-8def-7123456789ab'
const STAGING_INSTANCE_UUID = '018f3f5a-1d2c-4abc-8def-8123456789ab'
const MUTATION_UUID = '018f3f5a-1d2c-4abc-8def-9123456789ab'
...
function rawManifest(overrides: Record<string, unknown> = {}) {
	return {
		...
		snapshot: {
			commitSequence: 1,
			committedAt: NOW_TEXT,
			mutationId: MUTATION_UUID,
			vaultInstanceId: PREVIOUS_INSTANCE_UUID
		},
		vaultId: VAULT_UUID,
		...overrides
	}
...
function fixtures() {
	...
	const targetSchemaVersion = parseSchemaVersion(3)
	if (
		vaultId.isErr() ||
		otherVaultId.isErr() ||
		generationId.isErr() ||
	...
	return {
		...
		targetSchemaVersion: targetSchemaVersion.value,
...
function localCandidate(generationManifest = manifest()): GenerationCandidate {
	...
	const verification = createRuntimeLocalVerifiedGeneration({
		...
		objectChecksum,
		snapshot: generationManifest.snapshot,
...
function restoreAttempt(
	input: Parameters<typeof createRestoreAttempt>[0]
): RestoreAttempt {
...
function validatedCandidate(): ValidatedRestoreCandidate {
	const { targetSchemaVersion, vaultId } = fixtures()
	const candidate = validateRestoreCandidate(
		localCandidate(),
		targetSchemaVersion,
		vaultId
	)
...
function remoteReceipt(
	...
	const receipt = createRuntimeRemoteObjectReceipt({
		...
		providerObjectPath:
			overrides.providerObjectPath ??
			`vaults/${vaultId}/generations/${generationId}.lena`,
		verifiedAt: later,
...
function remoteUploadedAttempt(): BackupAttempt {
	const { effectId, generationId, later, now, objectChecksum, vaultId } =
		fixtures()
	return applyBackupEvents(
...
describe('generation manifests', () => {
	test('parses bounded versioned metadata', () => {
		...
		if (parsed.isErr()) throw parsed.error
		expect(parsed.value.payload.recordCounts).toEqual({
			entries: 4,
			settings: 2
		})
	...
	test('rejects corrupt metadata before payload allocation', () => {
		...
		expect(
			parseGenerationManifest(
				rawManifest({ completedAt: NOW_TEXT, createdAt: LATER_TEXT })
			).isOk()
		).toBe(false)
		expect(
			parseGenerationManifest(
				rawManifest({ reason: 'restore-purchases' })
			).isOk()
		).toBe(false)
	})
	...
	test('rejects unknown metadata instead of accepting identity leakage', () => {
		expect(
			parseGenerationManifest(
				rawManifest({ email: 'person@example.com' })
			).isOk()
		).toBe(false)
	})
	...
	test('serializes deterministically before the manifest enters authenticated ciphertext', () => {
		...
		if (firstSerialized.isErr()) throw firstSerialized.error
		expect(
			parseGenerationManifest(JSON.parse(firstSerialized.value)).isOk()
		).toBe(true)
	})
		...
		fc.assert(
			fc.property(
				...
				({ attachmentCount, byteLengths, reason, recordCounts }) => {
					...
					if (reparsed.isErr()) throw reparsed.error
					expect(reparsed.value).toEqual(parsed.value)
					expect(serializeGenerationManifest(reparsed.value)).toEqual(
						serialized
					)
				}
			),
...
describe('verified generation evidence', () => {
	test('round-trips a strict branded remote receipt', () => {
		...
		const parsed = parsePersistedRemoteObjectReceiptClaim(
			JSON.parse(JSON.stringify(receipt))
		)
		expect(parsed.isOk()).toBe(true)
		if (parsed.isOk())
			expect(isRuntimeRemoteObjectReceipt(parsed.value)).toBe(false)
		expect(
			parsePersistedRemoteObjectReceiptClaim({
				...receipt,
	...
	test('binds remote verification to the exact object', () => {
		...
		const base = {
			...
			sourceVerification: runtimeLocalVerification()
		}
		expect(
			createRuntimeRemoteVerifiedGeneration({
				...base,
		...
		expect(
			createRuntimeRemoteVerifiedGeneration({
				...base,
				receipt: remoteReceipt({
					providerObjectPath: 'vaults/wrong/generation.lena'
				})
			}).isOk()
		).toBe(false)
		expect(
	...
	test('rejects forged verification provenance while parsing persistence', () => {
		...
		if (local === null) throw new Error('Expected local verification')
		expect(
			parsePersistedGenerationVerificationClaim(
...
describe('backup lifecycle', () => {
	test('requires the active claim and exact receipt before verification', () => {
		...
		if (verified.value.state !== 'verified')
	...
	test('rejects stale claims at upload recording and verification', () => {
		...
		const persisted = parseBackupAttempt(
			JSON.parse(JSON.stringify(remoteUploadedAttempt()))
		)
	...
	test('retry releases stale upload ownership and reacquires a claim', () => {
		...
		const retried = reduceBackupAttempt(failed.value, {
			at: later,
			type: 'retry'
		})
	...
	test('rejects skipped local verification and arbitrary failure codes', () => {
		...
		const attempt = createBackupAttempt({
			createdAt: now,
			generationId,
			vaultId
		})
	...
	test('strictly parses durable states and rejects cross-generation verification', () => {
		...
		if (verified.isErr()) throw verified.error
		expect(
			parseBackupAttempt(JSON.parse(JSON.stringify(verified.value))).isOk()
		).toBe(true)
		expect(
			parseBackupAttempt({
				...verified.value,
		...
		expect(
			parseBackupAttempt({ ...verified.value, rawError: 'secret' }).isOk()
		).toBe(false)
	})
		...
		fc.assert(
			fc.property(
				...
				(millisecondsEarlier) => {
					...
					if (earlier.isErr()) throw earlier.error
					...
					if (result.isOk()) throw new Error('Expected stale event rejection')
...
describe('discovery', () => {
	...
	test('requires exact branded verification and schema compatibility', () => {
		const { objectChecksum, targetSchemaVersion, vaultId } = fixtures()
		const candidate = localCandidate()
		expect(
			validateRestoreCandidate(candidate, targetSchemaVersion, vaultId).isOk()
		).toBe(true)
		expect(
			validateRestoreCandidate(candidate, 0 as never, vaultId).isErr()
		).toBe(true)
		expect(
			validateRestoreCandidate(
		...
		const persisted = createPersistedLocalVerificationClaim({
			...
			objectByteLength: 240,
			objectChecksum,
			snapshot: candidate.manifest.snapshot,
			...
			validateRestoreCandidate(
				{
					...candidate,
					verification:
						persisted.value as unknown as GenerationCandidate['verification']
				},
				targetSchemaVersion,
				vaultId
			).isOk()
	...
	test('classifies older schemas for staging and rejects future schemas', () => {
		const { targetSchemaVersion, vaultId } = fixtures()
		const older = validateRestoreCandidate(
			localCandidate(manifest({ schemaVersion: 1 })),
			targetSchemaVersion,
			vaultId
		)
		expect(older.isOk()).toBe(true)
		if (older.isErr()) throw older.error
		expect(older.value.schemaDisposition).toBe('requires-migration')
		expect(older.value.targetSchemaVersion).toBe(targetSchemaVersion)

		const current = validateRestoreCandidate(
			localCandidate(manifest({ schemaVersion: 3 })),
			targetSchemaVersion,
			vaultId
		)
		expect(current.isOk()).toBe(true)
		if (current.isErr()) throw current.error
		expect(current.value.schemaDisposition).toBe('current')

		const future = validateRestoreCandidate(
			localCandidate(manifest({ schemaVersion: 4 })),
			targetSchemaVersion,
			vaultId
		)
		expect(future.isErr()).toBe(true)
		if (future.isOk()) throw new Error('Expected future schema rejection')
		expect(future.error.code).toBe('incompatible_schema')
	})

	test('scopes remote verification to its provider', () => {
		const { effectId, later, targetSchemaVersion, vaultId } = fixtures()
		const verified = reduceBackupAttempt(remoteUploadedAttempt(), {
			at: later,
			claimId: effectId,
			receipt: remoteReceipt(),
		...
		if (verified.value.state !== 'verified')
			throw new Error('Expected verified backup')
		if (!isRuntimeVerifiedGeneration(verified.value.verification)) {
			throw new Error('Expected runtime verification')
		}
	...
	test('rejects restore into a different logical vault', () => {
		const { otherVaultId, targetSchemaVersion } = fixtures()
		const result = validateRestoreCandidate(
			localCandidate(),
			targetSchemaVersion,
			otherVaultId
		)
...
describe('staged restore', () => {
	test('supports fresh-device restore with no previous physical pointer', () => {
		...
		expect(restored.targetSchemaVersion).toBe(candidate.targetSchemaVersion)
	})

	test('binds staging readiness to the exact host target schema', () => {
		const { now, stagingInstanceId } = fixtures()
		const candidate = validatedCandidate()
		const generationBinding = binding(candidate)
		const built = applyRestoreEvents(
			restoreAttempt({
				candidate,
				discoveredAt: now,
				previousInstanceId: null
		...
		const wrongTarget = parseSchemaVersion(2)
		if (wrongTarget.isErr()) throw wrongTarget.error
		const mismatchedReady = reduceRestoreAttempt(built, {
			at: now,
			targetSchemaVersion: wrongTarget.value,
			type: 'record-ready'
		})
		expect(mismatchedReady.isErr()).toBe(true)
		if (mismatchedReady.isOk())
			throw new Error('Expected target schema mismatch')
		expect(mismatchedReady.error.code).toBe('incompatible_schema')

		expect(
			parseRestoreAttempt({ ...built, targetSchemaVersion: 1 }).isErr()
		).toBe(true)
	})
	...
	test('persists uncertain activation and retries rollback', () => {
		...
		expect(
			parseRestoreAttempt(JSON.parse(JSON.stringify(unknown.value))).isOk()
		).toBe(true)
	...
	test('makes rollback failure durable and retryable', () => {
		...
		expect(
			parseRestoreAttempt(JSON.parse(JSON.stringify(failed.value))).isOk()
		).toBe(true)
		expect(
			reduceRestoreAttempt(failed.value, {
				at: now,
				type: 'begin-rollback'
			}).isOk()
		).toBe(true)
	...
	test('allows enumerated pre-activation failure but rejects raw errors', () => {
		...
		expect(failed.isOk()).toBe(true)
		if (failed.isErr()) throw failed.error
		if (failed.value.state !== 'failed')
			throw new Error('Expected failed restore')
...
describe('retention', () => {
	test('validates all retention policy counts', () => {
		expect(
			validateRetentionPolicy({ daily: 7, monthly: 12, recent: 3 }).isOk()
		).toBe(true)
		expect(
			validateRetentionPolicy({ daily: -1, monthly: 12, recent: 3 }).isOk()
		).toBe(false)
		expect(
			validateRetentionPolicy({ daily: 1.5, monthly: 12, recent: 3 }).isOk()
		).toBe(false)
		expect(
			validateRetentionPolicy({ daily: 10_001, monthly: 12, recent: 3 }).isOk()
		).toBe(false)
		expect(
	...
	test('keeps newest verified and matching LKG before deleting older data', () => {
		...
		const newest = manifest({
			completedAt: LATEST_TEXT,
			generationId: THIRD_GENERATION_UUID
		})
	...
	test('rejects persisted evidence and copied plans as deletion authority', () => {
		...
		const forged = parsed.value as unknown as ReturnType<
			typeof runtimeLocalVerification
		>
		expect(
			planGenerationRetention(
				[
