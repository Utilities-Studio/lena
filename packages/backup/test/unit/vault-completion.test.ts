...
import type { BackupOutboxRow, SyncVaultDatabase } from '@lena-inc/vault'
import {
...
describe('verified-generation ledger completion', () => {
	test('records the verified watermark and satisfies its covered claim in one transaction', () => {
		...
		const selected: Array<BackupOutboxRow | undefined> = [
			pendingRow(fixture),
			target,
			undefined
		]
		...
		const database = {
			transaction: (
				callback: (transaction: unknown) => BackupOutboxRow,
				config: unknown
			) => {
