...
describe("index identity and vectors", () => {
  test("canonicalizes tokenizer option order", () => {
    ...
    expect(second.isOk() && isSameSearchIndexIdentity(first, second.value)).toBe(true);
  ...
  test("validates vector dimensions and finite components", () => {
    ...
    expect(validateEmbeddingForIndex([1, 2, 3], metadata.value).isOk()).toBe(true);
    expect(validateEmbeddingForIndex([1, 2], metadata.value).isOk()).toBe(false);
    expect(validateEmbeddingForIndex([1, Number.POSITIVE_INFINITY, 3], metadata.value).isOk()).toBe(
      false,
    );
  });
});
