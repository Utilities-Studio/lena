...
describe("lexical normalization", () => {
  ...
  test("rejects non-finite scores", () => {
    expect(normalizeFtsResults([{ documentId: "a", score: Number.NaN }]).isOk()).toBe(false);
  });
});
...
describe("reciprocal rank fusion", () => {
  ...
  test("uses document id as the final stable tie break", () => {
    ...
    expect(fused.isOk() && fused.value.map(({ documentId }) => documentId)).toEqual(["a", "z"]);
  });
  ...
  test("supports a missing modality and enforces weight bounds", () => {
    ...
    expect(fuseHybridResults({ lexical: [], lexicalWeight: 1.1, semantic: [] }).isOk()).toBe(false);
    expect(
      fuseHybridResults({
        lexical: [],
        lexicalWeight: 0,
