...
const rankedDocumentSchema = z
	.strictObject({ documentId: documentIdSchema })
	.readonly()
...
export function normalizeFtsResults(
	...
	const resultArray = z
		.array(lexicalResultSchema)
		.max(MAX_RANKING_RESULTS)
		.safeParse(value)
...
export function fuseHybridResults(
	...
	const fused = documentIds.map((documentId) => {
		...
		const score =
			...
			(semanticRank === null
				? 0
				: semanticWeight / (rankConstant + semanticRank))
