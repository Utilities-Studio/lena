...
export function transitionSearchIndexRebuild(
	...
	return match(inputEvent)
		.with({ type: 'rebuild_requested' }, (event) => {
			...
			const sourceRevision = parseOpaqueValue(
		...
		.with({ type: 'batch_committed' }, (event) => {
			...
			const upsertedDocuments =
				state.checkpoint.upsertedDocuments + event.upsertedDocuments
			const removedDocuments =
				state.checkpoint.removedDocuments + event.removedDocuments
			if (
				!Number.isSafeInteger(upsertedDocuments) ||
				!Number.isSafeInteger(removedDocuments)
			) {
...
function requireActiveRebuild(
	...
	const eventRebuildId = parseOpaqueValue(
		eventRebuildIdValue,
		'Rebuild id',
		256
	)
...
function findPreviousReady(
	state: SearchIndexRebuildState
): ReadySearchIndexState | null {
