...
export const storeKitFactSourceSchema = z.enum([
	'launch',
	'restore',
	'transaction_update'
])
...
export type StoreKitEntitlementDisposition = z.infer<
	typeof storeKitEntitlementDispositionSchema
>
export type StoreKitFactSource = z.infer<typeof storeKitFactSourceSchema>

const verifiedStoreKitFactBrand: unique symbol = Symbol(
...
export function parseStoreKitSequence(
	value: unknown
): Result<StoreKitSequence, LenaError> {
	const parsed = storeKitSequenceSchema.safeParse(value)
	if (!parsed.success) {
		return err(
			new LenaError('invalid_input', { boundary: 'storekit_sequence' })
		)
	}
...
export function isVerifiedStoreKitEntitlementFact(
	...
	return (
		...
		(input as { readonly [verifiedStoreKitFactBrand]?: unknown })[
			verifiedStoreKitFactBrand
		] === true
	)
}
...
export function storeKitFactFingerprint(
