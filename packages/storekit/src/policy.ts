...
export const storeKitProductTypeSchema = z.enum([
...
export function parseStoreKitProductId(
	value: unknown
): Result<StoreKitProductId, LenaError> {
...
export function createStoreKitProductPolicy(
	...
	const parsed = storeKitProductPolicySchema.safeParse({
		productId,
		productType
	})
	if (!parsed.success) {
