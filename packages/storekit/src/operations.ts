...
export type StoreKitOperationFailure = z.infer<typeof storeKitOperationFailureSchema>;
...
export const INITIAL_STOREKIT_PURCHASE_STATE: StoreKitPurchaseState = Object.freeze({
...
function activeFactAt(
  ...
  return (
    ((policy.productType === "non-consumable" && fact.expiresAt === null) ||
      (policy.productType === "auto-renewable-subscription" && fact.expiresAt !== null)) &&
...
export function reduceStoreKitPurchase(
  ...
  return match<[StoreKitPurchaseState, StoreKitPurchaseEvent]>([state, event])
    ...
    .with([{ status: P.union("purchasing", "pending") }, { type: "pending" }], () =>
      ok(Object.freeze({ status: "pending" as const })),
    )
    .with([{ status: P.union("purchasing", "pending") }, { type: "cancelled" }], () =>
      ok(Object.freeze({ status: "cancelled" as const })),
    )
    .with([{ status: P.union("purchasing", "pending") }, { type: "failed" }], ([, failedEvent]) => {
      const failure = storeKitOperationFailureSchema.safeParse(failedEvent.failure);
...
export const INITIAL_STOREKIT_RESTORE_STATE: StoreKitRestoreState = Object.freeze({
  status: "idle" as const,
});
...
export function reduceStoreKitRestore(
  ...
  return match<[StoreKitRestoreState, StoreKitRestoreEvent]>([state, event])
    ...
    .with([{ status: "restoring" }, { type: "completed" }], ([, completedEvent]) => {
      ...
      const latest = selectLatestStoreKitFact(
        completedEvent.facts.filter((fact) => fact.productId === policy.productId),
      );
      if (latest === null || !activeFactAt(latest, policy, observedAt.value)) {
