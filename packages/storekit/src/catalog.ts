...
const runtimeStoreKitCatalogStateBrand: unique symbol = Symbol(
	'RuntimeStoreKitCatalogState'
)
...
function catalogIdentity(
	products: readonly StoreKitProductPolicy[]
): StoreKitCatalogId {
...
export function createStoreKitEntitlementCatalog(
	...
	for (const inputProduct of parsedInput.data) {
		const product = createStoreKitProductPolicy(
			inputProduct.productId,
			inputProduct.productType
		)
		if (product.isErr()) {
...
function validateCatalogFacts(
	...
	for (const fact of facts) {
		...
		if (
			...
			(product.productType === 'auto-renewable-subscription' &&
				fact.expiresAt === null)
		) {
			return err(
				new LenaError('invalid_input', {
...
function processedEventResult(
	...
	const existing = state.processedEvents.find(
	...
	const runtimeState: StoreKitCatalogEntitlementState = {
		...
		latestSnapshot:
			state.latestSnapshot === null
				? null
				: Object.freeze({ ...state.latestSnapshot }),
...
export function reduceStoreKitCatalogEntitlement(
	...
	return match(authorizedEvent)
		.with({ type: 'snapshot' }, (event) => {
			if (
				state.latestSnapshot !== null &&
				event.sequence < state.latestSnapshot.sequence
			) {
				return ok(state)
			}
			...
			const currentFacts = currentFactPerProduct([
				...event.facts,
				...transactionFacts
			])
			...
			return ok(
				freezeState({
					...
					latestUnavailable:
						state.latestUnavailable !== null &&
						state.latestUnavailable.sequence > event.sequence
		...
		.with({ type: 'transaction' }, (event) => {
			if (
				state.latestSnapshot !== null &&
				sequence <= state.latestSnapshot.sequence
			) {
				return ok(state)
			}
			...
			return ok(
				freezeState({
					...state,
					currentFacts: currentFactPerProduct([
						...state.snapshotFacts,
						...transactionFacts
					]),
		...
		.with({ type: 'unavailable' }, (event) => {
			...
			const latestUnavailable =
				state.latestUnavailable === null ||
				event.sequence > state.latestUnavailable.sequence
					? event
					: state.latestUnavailable
			return ok(
				freezeState({
...
function latestStateObservation(
	state: StoreKitCatalogEntitlementState
): IsoTimestamp | null {
	const observations = [
		...
		...(state.latestUnavailable === null
			? []
			: [state.latestUnavailable.observedAt])
	]
	return (
		maxBy(observations, (timestamp) => parseISO(timestamp).getTime()) ?? null
	)
}
...
export function evaluateStoreKitCatalogEntitlement(
	...
	if (
		...
		compareIsoTimestamps(asOf.value, latestObservation) < 0
	) {
		return err(
			new LenaError('invalid_input', {
				boundary: 'storekit_catalog'
	...
	const active = orderBy(
		...
		[
			(fact) =>
				findStoreKitCatalogProduct(catalog, fact.productId)?.productType ===
				'non-consumable'
					? 1
					: 0,
			(fact) => fact.sequence
...
export function hasStoreKitCatalogPaidAccess(
	...
	return authority.isOk()
		? ok(authority.value.kind === 'entitled')
		: err(authority.error)
}
