...
function policy(
  ...
  productType: "auto-renewable-subscription" | "non-consumable" = "non-consumable",
...
describe("native-verified StoreKit facts", () => {
  test("keeps the raw native trust constructor out of the package root", () => {
    expect("createVerifiedStoreKitEntitlementFactFromNativeAdapter" in storeKitRoot).toBe(false);
  ...
  test("uses runtime opacity that does not survive structural forgery or serialization", () => {
    ...
    expect(createStoreKitTransactionEventFromNativeAdapter(forged).isOk()).toBe(false);

    const serialized = JSON.parse(JSON.stringify(verified)) as VerifiedStoreKitEntitlementFact;
    expect(isVerifiedStoreKitEntitlementFact(serialized)).toBe(false);

    const issuedEvent = createStoreKitTransactionEventFromNativeAdapter(verified);
  ...
  test("requires explicit product type and positive monotonic sequences", () => {
    expect(createStoreKitProductPolicy(LIFETIME_PRODUCT, undefined).isOk()).toBe(false);
  ...
  test("keeps rejected native identifiers out of safe diagnostics", () => {
    ...
    if (rejected.isErr()) {
      expect(JSON.stringify(rejected.error.toJSON())).not.toContain(sensitiveProduct);
    }
  });
});
...
describe("purchase and explicit restore operations", () => {
  test("models pending and cancellation without granting", () => {
    ...
    const pending = reduceStoreKitPurchase(started.value, { type: "pending" }, policy());
    if (pending.isErr()) throw pending.error;
    const cancelled = reduceStoreKitPurchase(pending.value, { type: "cancelled" }, policy());
...
test("a forged fact cannot enter the catalog reducer", () => {
  ...
  const reduced = reduceStoreKitCatalogEntitlement(
    state,
    { fact: forged, type: "transaction" } as unknown as StoreKitEntitlementEvent,
    catalog.value,
  );
