...
function transaction(
	factValue: VerifiedStoreKitEntitlementFact
...
describe('exact catalog and catalog identity', () => {
	...
	test('rejects duplicate, malformed, and missing product types', () => {
		...
		expect(createStoreKitEntitlementCatalog([lifetime, lifetime]).isOk()).toBe(
			false
		)
		expect(
			createStoreKitEntitlementCatalog([
	...
	test('rejects state reuse across catalog changes and supports explicit reinitialization', () => {
		...
		expect(reduceStoreKitCatalogEntitlement(state, event, changed).isOk()).toBe(
			false
		)
		expect(
			evaluateStoreKitCatalogEntitlement(
...
describe('monotonic sequence and deterministic reduction', () => {
	test('preserves a newer transaction when a stale snapshot arrives later', () => {
		...
		const transactionFirst = reduce(
			[lifetimeActive, staleEmptySnapshot],
			configuredCatalog
		)
		const snapshotFirst = reduce(
			[staleEmptySnapshot, lifetimeActive],
			configuredCatalog
		)
		expect(transactionFirst).toEqual(snapshotFirst)
		expect(
			paid(transactionFirst, configuredCatalog, '2026-09-20T10:00:00.000Z')
		).toBe(true)
	})
	...
	test('a newer complete snapshot clears older transactions and blocks stale replay', () => {
		...
		const denied = reduce(
			[lifetimeActive, newerEmptySnapshot],
			configuredCatalog
		)
		expect(paid(denied, configuredCatalog, '2026-09-30T10:00:00.000Z')).toBe(
			false
		)

		const replay = reduceStoreKitCatalogEntitlement(
			denied,
			lifetimeActive,
			configuredCatalog
		)
	...
	test('live permutations and batch reduction converge for cross-product events', () => {
		...
		for (const event of reverse([...events])) {
			const next = reduceStoreKitCatalogEntitlement(
				live,
				event,
				configuredCatalog
			)
	...
	test('rejects caller-minted snapshot resurrection while adapter sequence 3 stays deterministic', () => {
		...
		expect(
			paid(revokedState, configuredCatalog, '2026-09-02T10:00:00.000Z')
		).toBe(false)
		...
		) as ReturnType<typeof createInitialStoreKitCatalogEntitlementState>
		expect(
			evaluateStoreKitCatalogEntitlement(
				serializedState,
				configuredCatalog,
		...
		const chronological = reduce(
			[activeEvent, revokedEvent, adapterSnapshot],
			configuredCatalog
		)
		const outOfOrder = reduce(
			[adapterSnapshot, revokedEvent, activeEvent],
			configuredCatalog
		)
		expect(outOfOrder).toEqual(chronological)
		expect(
			paid(chronological, configuredCatalog, '2026-09-03T10:00:00.000Z')
		).toBe(false)
	})
})
...
describe('multi-product authority and offline expiry', () => {
	...
	test('expires an annual entitlement from verified expiresAt while offline', () => {
		...
		expect(
			paid(onlineState, configuredCatalog, '2026-09-09T09:00:00.000Z')
		).toBe(true)

		const state = reduce([annualActive, unavailable.value], configuredCatalog)
		expect(paid(state, configuredCatalog, '2026-09-10T09:00:00.000Z')).toBe(
			false
		)
		const authority = evaluateStoreKitCatalogEntitlement(
			state,
	...
	test('keeps verified lifetime access offline but fails closed without verified state', () => {
		...
		expect(
			paid(lifetimeState, configuredCatalog, '2026-09-02T10:00:00.000Z')
		).toBe(true)

		const unavailableOnly = reduce([unavailable.value], configuredCatalog)
		expect(
			paid(unavailableOnly, configuredCatalog, '2026-09-02T10:00:00.000Z')
		).toBe(false)
		const authority = evaluateStoreKitCatalogEntitlement(
			unavailableOnly,
			configuredCatalog,
	...
	test('rejects an as-of time that predates observed authority state', () => {
		const configuredCatalog = catalog()
		const state = reduce(
			[transaction(fact(policies().lifetime, 1))],
			configuredCatalog
		)
		expect(
			hasStoreKitCatalogPaidAccess(
