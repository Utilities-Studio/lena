...
describe('iCloud transport protocol', () => {
	test('plans upload only from exact runtime local evidence', () => {
		...
		expect(
			createICloudImmutableUploadPlan(generation, local, { ...attempt }).isOk()
		).toBe(false)
	...
	test('reconciles a conflict only for the exact runtime receipt, target, path, and claim', () => {
		...
		if (upload.isErr() || target.isErr())
			throw new Error('Invalid conflict fixture')

		expect(
			reconcileICloudUploadConflict(upload.value, target.value, receipt).isOk()
		).toBe(true)
	...
	test('lists read-only, inspects exact ids, and downloads runtime-verified objects to staging', () => {
		...
		expect(createICloudInspectPlan(generation, 'icloud-object-1').isOk()).toBe(
			true
		)

		const remote = runtimeRemoteVerification(generation)
		const download = createICloudDownloadPlan(
			generation,
			remote,
	...
	test('deletes only with runtime verification and runtime-backed retention', () => {
		...
		expect(
			createICloudDeletePlan(generation, remote, { ...retention }).isOk()
		).toBe(false)
		...
		const wrongPathRemote = runtimeRemoteVerification(
			generation,
			wrongPathReceipt
		)
		expect(
			createICloudDeletePlan(generation, wrongPathRemote, retention).isOk()
		).toBe(false)
	})
	...
	test('classifies account, quota, and unknown runtime failures safely', () => {
		...
		expect(classifyUnknownICloudFailure('future-provider-code').kind).toBe(
			'fatal'
		)
	})
})
