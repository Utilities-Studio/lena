    ...
    fc.assert(
      fc.property(
        ...
        (vaultId, vaultInstanceId, mutationId, createdAt) => {
          ...
          const parsed = parseBackupObligation(JSON.parse(JSON.stringify(persisted)));
          expect(parsed.isOk()).toBe(true);
          if (parsed.isErr()) return;
          expect(backupObligationSchema.safeParse(parsed.value).success).toBe(true);
    ...
    fc.assert(
      fc.property(fc.nat({ max: 1_000 }), (attemptCount) => {
        const candidate = {
          ...
          mutationId: "018f3f5a-1d2c-7abc-8def-2123456789ab",
          state: "pending",
          vaultId: "018f3f5a-1d2c-7abc-8def-0123456789ab",
          vaultInstanceId: "018f3f5a-1d2c-7abc-8def-1123456789ab",
        };

        expect(parseBackupObligation(candidate).isOk()).toBe(attemptCount === 0);
        expect(pendingBackupObligationSchema.safeParse(candidate).success).toBe(attemptCount === 0);
        expect(parseBackupObligation({ ...candidate, providerToken: "forbidden" }).isOk()).toBe(
          false,
        );
      }),
    );
