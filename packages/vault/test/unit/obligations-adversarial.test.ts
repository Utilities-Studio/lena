...
function claimedObligation(): ClaimedBackupObligation {
	...
	const claimed = claimBackupObligation(
		pending,
		fixture.effectId,
		fixture.claimedAt
	)
...
describe('persisted backup obligations', () => {
	test('round trips pending and claimed states with physical instance identity', () => {
		const fixture = obligationFixtures()
		const pending = createBackupObligation(fixture)
		expect(parseBackupObligation(JSON.parse(JSON.stringify(pending)))).toEqual(
			ok(pending)
		)
		const claimed = claimBackupObligation(
			pending,
		...
		if (claimed.isErr()) throw claimed.error
		expect(
			parseBackupObligation(JSON.parse(JSON.stringify(claimed.value)))
		).toEqual(claimed)
	})
	...
	test('rejects incoherent fields, unknown fields, and arbitrary failure codes', () => {
		...
		expect(parseBackupObligation({ ...pending, attemptCount: 1 }).isOk()).toBe(
			false
		)
		expect(
			parseBackupObligation({ ...pending, claimId: fixture.effectId }).isOk()
		).toBe(false)
		expect(
			parseBackupObligation({
				...claimed.value,
...
test('the vault package does not import the backup package', async () => {
	...
	expect(sources.join('\n')).not.toMatch(/from\s+["']@lena-inc\/backup/)
})
