...
function getEntry(registry: VaultRegistry, instanceId: VaultRegistryEntry["vaultInstanceId"]) {
  const entry = registry.entries.find((candidate) => candidate.vaultInstanceId === instanceId);
...
describe("validated physical vault lifecycle", () => {
  test("requires staging, validation, readiness, and activation evidence", () => {
    ...
    const second = addVaultRegistryEntry(first.value, entry(instanceB, "vaults/restored.sqlite"));
    if (second.isErr()) throw second.error;

    const bothReady = validateAndReady(validateAndReady(second.value, instanceA), instanceB);
  ...
  test("generic updates cannot change staging, ready, or retired state", () => {
    ...
    if (added.isErr()) throw added.error;
    expect(updateRegisteredVaultState(added.value, instanceA, "ready").isOk()).toBe(false);
    expect(updateRegisteredVaultState(added.value, instanceA, "retired").isOk()).toBe(false);

    const ready = validateAndReady(added.value, instanceA);
    expect(updateRegisteredVaultState(ready, instanceA, "staging").isOk()).toBe(false);
    expect(updateRegisteredVaultState(ready, instanceA, "retired").isOk()).toBe(false);
    ...
    expect(updateRegisteredVaultState(retired.value, instanceA, "ready").isOk()).toBe(false);
    expect(updateRegisteredVaultState(retired.value, instanceA, "staging").isOk()).toBe(false);
  });
  ...
  test("validation and activation tokens cannot be forged or survive JSON", () => {
    ...
    const jsonValidation = JSON.parse(JSON.stringify(validation.value)) as VaultValidationToken;
    expect(isVaultValidationToken(spreadValidation)).toBe(false);
    expect(isVaultValidationToken(jsonValidation)).toBe(false);
    expect(markStagingVaultReady(added.value, spreadValidation).isOk()).toBe(false);
    expect(markStagingVaultReady(added.value, jsonValidation).isOk()).toBe(false);

    const ready = markStagingVaultReady(added.value, validation.value);
    if (ready.isErr()) throw ready.error;
    expect(markStagingVaultReady(added.value, validation.value).isOk()).toBe(false);
    const activation = activationFor(ready.value, instanceA);
    if (activation.isErr()) throw activation.error;
    const spreadActivation = { ...activation.value } as VaultActivationToken;
    const jsonActivation = JSON.parse(JSON.stringify(activation.value)) as VaultActivationToken;
    expect(isVaultActivationToken(spreadActivation)).toBe(false);
    expect(isVaultActivationToken(jsonActivation)).toBe(false);
    expect(activateRegisteredVault(ready.value, spreadActivation).isOk()).toBe(false);
    expect(activateRegisteredVault(ready.value, jsonActivation).isOk()).toBe(false);
    const active = activateRegisteredVault(ready.value, activation.value);
    if (active.isErr()) throw active.error;
    expect(activateRegisteredVault(ready.value, activation.value).isOk()).toBe(false);
  });
  ...
  test("retirement requires an exact runtime-active replacement and is one-shot", () => {
    ...
    expect(getSelectedVaultEntryForReopen(restarted.value)?.vaultInstanceId).toBe(instanceB);
    expect(getActiveVaultEntry(restarted.value)).toBeNull();
    expect(retireRegisteredVault(restarted.value, retirement.value).isOk()).toBe(false);

    const retiringInstanceActive = activate(replacementActive, instanceA);
    expect(retireRegisteredVault(retiringInstanceActive, retirement.value).isOk()).toBe(false);
    const replacementReactivated = activate(retiringInstanceActive, instanceB);
    const retired = retireRegisteredVault(replacementReactivated, retirement.value);
    if (retired.isErr()) throw retired.error;
    ...
    expect(getSelectedVaultEntryForReopen(retired.value)?.vaultInstanceId).toBe(instanceB);
    expect(retireRegisteredVault(replacementReactivated, retirement.value).isOk()).toBe(false);
  });
  ...
  test("retirement tokens cannot be forged, spread, or revived from JSON", () => {
    ...
    const parsedJson = JSON.parse(JSON.stringify(retirement.value)) as VaultRetirementToken;
  ...
  test("retirement evidence rejects every target and replacement identity mismatch", () => {
    const { entry, instanceA, instanceB, instanceC, nextSchemaVersion } = registryFixtures();
    ...
    for (const changedTarget of targetVariants) {
      const changed = parseAndActivate([changedTarget, replacement], instanceB, later);
      expect(retireRegisteredVault(changed, retirement.value).isOk()).toBe(false);
    }

    const replacementVariants: ReadonlyArray<
      readonly [VaultRegistryEntry, VaultRegistryEntry["vaultInstanceId"]]
    > = [
      [{ ...replacement, locator: "vaults/changed-replacement.sqlite" }, instanceB],
    ...
    for (const [changedReplacement, activeInstanceId] of replacementVariants) {
      const changed = parseAndActivate([target, changedReplacement], activeInstanceId, later);
      expect(retireRegisteredVault(changed, retirement.value).isOk()).toBe(false);
    }
  ...
  test("retirement issuance rejects same, cross-vault, missing, and non-ready instances", () => {
    const { createdAt, entry, instanceA, instanceB, instanceC, otherVaultId } = registryFixtures();
    ...
    const issue = (retiringEntry: VaultRegistryEntry, replacementEntry: VaultRegistryEntry) =>
      createVaultRetirementTokenFromAdapter({
    ...
    expect(issue({ ...target, state: "staging" }, replacement).isOk()).toBe(false);
    expect(issue({ ...target, state: "retired" }, replacement).isOk()).toBe(false);
    expect(issue(target, { ...replacement, state: "staging" }).isOk()).toBe(false);
    expect(issue(target, { ...replacement, state: "retired" }).isOk()).toBe(false);
    expect(issue(target, entry(instanceC, "vaults/missing.sqlite", "ready")).isOk()).toBe(false);

    const crossVaultReplacement = {
      ...entry(instanceB, "vaults/cross-vault.sqlite", "ready"),
      vaultId: otherVaultId,
    };
    const crossVault = parseAndActivate([target, crossVaultReplacement], instanceB);
  ...
  test("a token fails closed for the wrong active replacement without being consumed", () => {
    ...
    const retirement = retirementFor(expectedReplacementActive, instanceA, instanceB);
    if (retirement.isErr()) throw retirement.error;

    const wrongReplacementActive = activate(expectedReplacementActive, instanceC);
    expect(retireRegisteredVault(wrongReplacementActive, retirement.value).isOk()).toBe(false);
    expect(isVaultRetirementToken(retirement.value)).toBe(true);

    const expectedReplacementReactivated = activate(wrongReplacementActive, instanceB);
    expect(retireRegisteredVault(expectedReplacementReactivated, retirement.value).isOk()).toBe(
      true,
    );
  });
  ...
  test("retirement issuance validates cleanup and replacement chronology", () => {
    ...
    const target = getEntry(active, instanceA);
    const replacement = getEntry(active, instanceB);
    const issue = (cleanupAuthorizedAt: typeof createdAt, validatedAt: typeof createdAt) =>
      createVaultRetirementTokenFromAdapter({
        cleanupAuthorizedAt,
        registry: active,
    ...
    expect(issue("not-a-timestamp" as typeof createdAt, createdAt).isOk()).toBe(false);
  ...
  test("evidence is bound to exact identity, metadata, and schema", () => {
    const { createdAt, entry, instanceA, instanceB, nextSchemaVersion, vaultId } =
      registryFixtures();
    ...
    if (wrongInstance.isErr() || wrongSchema.isErr()) throw new Error("Bad metadata fixture");
    expect(
      createVaultValidationTokenFromAdapter({
        entry: staging,
...
describe("registry structure and persistence", () => {
  test("rejects duplicate instances, duplicate locators, and unsafe locators", () => {
    ...
    if (first.isErr()) throw first.error;
    expect(addVaultRegistryEntry(first.value, entry(instanceA, "vaults/other.sqlite")).isOk()).toBe(
      false,
    );
    expect(
      addVaultRegistryEntry(first.value, entry(instanceB, "vaults/primary.sqlite")).isOk(),
    ).toBe(false);
  ...
  test("retains a restart selection hint without granting runtime activation", () => {
    ...
    expect(getSelectedVaultEntryForReopen(parsedReady.value)?.vaultInstanceId).toBe(instanceA);

    const forgedRuntimeRegistry = { ...parsedReady.value } as VaultRegistry;
    expect(getActiveVaultEntry(forgedRuntimeRegistry)).toBeNull();
    expect(getSelectedVaultEntryForReopen(forgedRuntimeRegistry)?.vaultInstanceId).toBe(instanceA);
    expect(updateRegisteredVaultState(parsedReady.value, instanceA, "retired").isOk()).toBe(false);
    expect(retirementFor(parsedReady.value, instanceA, instanceB).isOk()).toBe(false);

    const activation = activationFor(parsedReady.value, instanceA);
    if (activation.isErr()) throw activation.error;
    const reopened = activateRegisteredVault(parsedReady.value, activation.value);
...
describe("metadata and registry identity", () => {
  test("requires logical id, physical instance id, and schema to match", () => {
    ...
    expect(validateVaultMetadataAgainstRegistryEntry(metadata, registryEntry).isOk()).toBe(true);
    ...
    expect(
      validateVaultMetadataAgainstRegistryEntry(wrongLogical.value, registryEntry).isOk(),
    ).toBe(false);
    expect(
      validateVaultMetadataAgainstRegistryEntry(wrongInstance.value, registryEntry).isOk(),
    ).toBe(false);
    expect(validateVaultMetadataAgainstRegistryEntry(wrongSchema.value, registryEntry).isOk()).toBe(
      false,
    );
  });
