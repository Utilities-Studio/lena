...
describe('canonical Lena Drizzle schema', () => {
	test('owns one mapping with exact SQL table and column names', () => {
		...
		expect(getTableConfig(lenaVaultMetadataTable).name).toBe(
			'lena_vault_metadata'
		)
		expect(
			getTableConfig(lenaVaultMetadataTable).columns.map(
				(column) => column.name
			)
		).toEqual([
			'singleton_id',
		...
		expect(
			getTableConfig(lenaBackupOutboxTable).columns.map((column) => column.name)
		).toEqual([
			'commit_sequence',
			'mutation_id',
			'vault_id',
		...
		expect(
			getTableConfig(lenaProcessedEffectsTable).columns.map(
				(column) => column.name
			)
		).toEqual([
			'effect_id',
		...
		expect(
			getTableConfig(lenaVerifiedBackupGenerationsTable).columns.map(
				(column) => column.name
			)
		).toEqual([
			'generation_id',
	...
	test('maps primary keys, defaults, enums, uniques, checks, and composite foreign keys', () => {
		...
		const verifiedGenerations = getTableConfig(
			lenaVerifiedBackupGenerationsTable
		)
		...
		expect(
			metadata.uniqueConstraints[0]?.columns.map((column) => column.name)
		).toEqual(['vault_id', 'vault_instance_id'])
		...
		expect(lenaBackupOutboxTable.state.enumValues).toEqual([
			'pending',
			'claimed',
			'satisfied'
		])
		expect(lenaProcessedEffectsTable.effectId.primary).toBe(true)
		expect(lenaProcessedEffectsTable.effectKind.enumValues).toEqual([
			'backup-obligation'
		])
		expect(lenaVerifiedBackupGenerationsTable.generationId.primary).toBe(true)
		expect(
			effects.uniqueConstraints[0]?.columns.map((column) => column.name)
		).toEqual(['vault_instance_id', 'effect_kind', 'mutation_id'])
		...
		expect(
			verifiedGenerations.foreignKeys[1]
				?.reference()
		...
		const verifiedGenerationChecks = renderChecks(
			lenaVerifiedBackupGenerationsTable
		)
...
describe('Lena-owned database row schemas', () => {
	test('validate metadata and processed-effect rows without exposing raw Zod errors', () => {
		...
		expect(parseVaultMetadataRow({ ...metadata, singletonId: 2 }).isErr()).toBe(
			true
		)
		expect(
			parseVaultMetadataRow({ ...metadata, providerToken: 'forbidden' }).isErr()
		).toBe(true)
	...
	test('enforces every outbox state and its chronology', () => {
		...
		expect(parseBackupOutboxRow({ ...pending, attemptCount: 1 }).isErr()).toBe(
			true
		)
		expect(
			parseBackupOutboxRow({
				...claimed,
				claimedAt: '2025-12-31T23:59:59.000Z'
			}).isErr()
		).toBe(true)
		expect(
			parseBackupOutboxRow({
				...satisfied,
				coveredCommitAt: COMPLETED_AT
			}).isErr()
		).toBe(true)
		expect(
			backupOutboxRowSchema.safeParse({ ...satisfied, receipt: 'forbidden' })
				.success
		).toBe(false)
	})
	...
	test('binds verified generations to an exact committed snapshot watermark', () => {
		...
		expect(
			parseVerifiedBackupGenerationRow({
				...row,
				verifiedAt: '2025-12-31T23:59:59.000Z'
			}).isErr()
		).toBe(true)
		expect(
			parseVerifiedBackupGenerationRow({ ...row, receipt: 'forbidden' }).isErr()
		).toBe(true)
	})
		...
		fc.assert(
			fc.property(fc.nat({ max: 1_000 }), (attemptCount) => {
				const valid = {
					...
					generationId: null,
					lastFailureCode:
						attemptCount === 0 ? null : 'temporarily_unavailable',
					mutationId: IDS.mutation,
					state: 'pending',
					vaultId: IDS.vault,
					vaultInstanceId: IDS.vaultInstance
				...
				expect(
					parseBackupOutboxRow({
						...valid,
						lastFailureCode:
							attemptCount === 0 ? 'temporarily_unavailable' : null
					}).isErr()
				).toBe(true)
			})
		)
