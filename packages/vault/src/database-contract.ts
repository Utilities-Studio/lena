...
export type VaultDatabaseCapabilities = z.infer<
	typeof vaultDatabaseCapabilitiesSchema
>

export const vaultDatabaseNameSchema = z
	.string()
	.regex(/^[a-z0-9][a-z0-9_-]{0,62}\.db$/)
...
export const REQUIRED_VAULT_OPEN_SEQUENCE: readonly VaultDatabaseOpenStep[] =
	Object.freeze([
		'open-with-key',
		'cipher-integrity-check',
		'enable-foreign-keys',
	...
	if (!parsed.success) {
		...
		return err(
			new LenaError('unsupported', {
				capability:
					typeof capability === 'string' ? capability : 'capability_shape'
			})
		)
	}
...
export function validateVaultDatabaseName(
	value: string
