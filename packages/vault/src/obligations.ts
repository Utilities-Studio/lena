...
import { z } from 'zod'
export const PERSISTABLE_LENA_ERROR_CODES: readonly LenaErrorCode[] =
	Object.freeze([...lenaErrorCodeSchema.options])

export const persistableLenaErrorCodeSchema = lenaErrorCodeSchema

export function parsePersistableLenaErrorCode(
	value: unknown
): Result<LenaErrorCode, LenaError> {
...
export const pendingBackupObligationSchema = z
	...
	.superRefine((obligation, context) => {
		...
		if (hasFailureCode && obligation.lastFailureCode === undefined) {
			...
			(obligation.lastFailureCode === undefined)
		) {
...
export const satisfiedBackupObligationSchema = z
	...
	.superRefine((obligation, context) => {
		if (
			...
			obligation.attemptCount < 1 ||
			compareIsoTimestamps(obligation.claimedAt, obligation.createdAt) < 0 ||
			compareIsoTimestamps(obligation.coveredCommitAt, obligation.committedAt) <
				0 ||
			compareIsoTimestamps(obligation.coveredCommitAt, obligation.claimedAt) >
				0 ||
			compareIsoTimestamps(obligation.completedAt, obligation.claimedAt) < 0 ||
			compareIsoTimestamps(obligation.completedAt, obligation.coveredCommitAt) <
				0
		) {
...
export type PendingBackupObligation = z.infer<
	typeof pendingBackupObligationSchema
>
export type ClaimedBackupObligation = z.infer<
	typeof claimedBackupObligationSchema
>
export type SatisfiedBackupObligation = z.infer<
	typeof satisfiedBackupObligationSchema
>
...
export function parseBackupObligation(
	value: unknown
): Result<BackupObligation, LenaError> {
	...
	const customIssue = parsed.error.issues.find(
