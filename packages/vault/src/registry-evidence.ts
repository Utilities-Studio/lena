...
export function isVaultValidationToken(
	input: unknown
): input is VaultValidationToken {
	return (
		typeof input === 'object' &&
		input !== null &&
		issuedVaultValidationTokens.has(input) &&
		!consumedVaultValidationTokens.has(input) &&
		(input as { readonly [vaultValidationTokenBrand]?: unknown })[
			vaultValidationTokenBrand
		] === true
	)
}

export function isVaultActivationToken(
	input: unknown
): input is VaultActivationToken {
	return (
		typeof input === 'object' &&
		input !== null &&
		issuedVaultActivationTokens.has(input) &&
		!consumedVaultActivationTokens.has(input) &&
		(input as { readonly [vaultActivationTokenBrand]?: unknown })[
			vaultActivationTokenBrand
		] === true
	)
}

export function isVaultRetirementToken(
	input: unknown
): input is VaultRetirementToken {
	return (
		typeof input === 'object' &&
		input !== null &&
		issuedVaultRetirementTokens.has(input) &&
		!consumedVaultRetirementTokens.has(input) &&
		(input as { readonly [vaultRetirementTokenBrand]?: unknown })[
			vaultRetirementTokenBrand
		] === true
	)
}

export function consumeVaultValidationToken(
	token: VaultValidationToken
): boolean {
...
export function consumeVaultActivationToken(
	token: VaultActivationToken
): boolean {
...
export function consumeVaultRetirementToken(
	token: VaultRetirementToken
): boolean {
...
export function createVaultRetirementTokenFromAdapter(
	...
	if (
		...
		input.replacementEntry.vaultInstanceId ===
			input.retiringEntry.vaultInstanceId
	) {
		return err(
			new LenaError('invalid_state_transition', {
				boundary: 'vault_registry_adapter'
	...
	if (
		...
		getRuntimeActiveVaultInstance(input.registry) !==
			input.replacementEntry.vaultInstanceId ||
		input.registry.activeVaultInstanceId !==
			input.replacementEntry.vaultInstanceId
	) {
		return err(
			new LenaError('invalid_state_transition', {
	...
	if (
		compareIsoTimestamps(cleanupAuthorizedAt.value, fields.value.validatedAt) <
			0 ||
		compareIsoTimestamps(
			cleanupAuthorizedAt.value,
			input.replacementEntry.createdAt
		) < 0
	) {
		return err(
			new LenaError('invalid_timestamp', {
