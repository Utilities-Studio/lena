...
export const lenaVaultMetadataTable = sqliteTable(
  ...
  (table) => [
    ...
    check("lena_vault_metadata_schema_version_check", sql`${table.schemaVersion} > 0`),
    check("lena_vault_metadata_format_version_check", sql`${table.formatVersion} > 0`),
    check(
      "lena_vault_metadata_encryption_envelope_version_check",
      sql`${table.encryptionEnvelopeVersion} > 0`,
    ),
    unique("lena_vault_metadata_vault_instance_unique").on(table.vaultId, table.vaultInstanceId),
  ],
);
...
export function parseVaultMetadataRow(value: unknown): Result<VaultMetadataRow, LenaError> {
  return parseDatabaseRow(vaultMetadataRowSchema, value, "lena_vault_metadata_row");
}

export function parseBackupOutboxRow(value: unknown): Result<BackupOutboxRow, LenaError> {
  return parseDatabaseRow(backupOutboxRowSchema, value, "lena_backup_outbox_row");
}
