...
export const vaultRegistryEntryStateSchema = z.enum([
	'ready',
	'retired',
	'staging'
])
...
export type VaultRegistryEntryState = z.infer<
	typeof vaultRegistryEntryStateSchema
>
...
export const vaultRegistrySchema = z
	...
	.superRefine((registry, context) => {
		...
		if (uniq(instanceIds).length !== instanceIds.length) {
			context.addIssue({
				code: 'custom',
				message: 'duplicate_vault_instance',
				path: ['entries']
			})
		}
		...
		if (uniq(locators).length !== locators.length) {
			context.addIssue({
				code: 'custom',
				message: 'duplicate_vault_locator',
				path: ['entries']
			})
		}
		...
		if (
			...
			!registry.entries.some(
				(entry) =>
					entry.vaultInstanceId === registry.activeVaultInstanceId &&
					entry.state === 'ready'
			)
		) {
			context.addIssue({
				code: 'custom',
				message: 'active_vault_not_ready',
				path: ['entries']
			})
		}
	})
...
function preserveRuntimeActivation(
	source: VaultRegistry,
...
export function validateVaultLocator(
	value: unknown
...
export function parseVaultRegistryEntry(
	value: unknown
...
function appendVaultRegistryEntry(
	...
	if (
		registry.entries.some(
			(item) => item.vaultInstanceId === entry.vaultInstanceId
		)
	) {
...
export function activateRegisteredVault(
	...
	const entry = registry.entries.find(
		(item) => item.vaultInstanceId === evidence.vaultInstanceId
	)
	if (!entry) {
		return err(new LenaError('not_found'))
	}
...
export function updateRegisteredVaultState(
	...
	const index = registry.entries.findIndex(
		(item) => item.vaultInstanceId === vaultInstanceId
	)
	if (index < 0) {
		return err(new LenaError('not_found'))
	}
...
export function retireRegisteredVault(
	...
	if (
		retiringEntry.state !== 'ready' ||
		!evidenceMatchesEntry(evidence, retiringEntry)
	) {
		return err(new LenaError('invalid_state_transition'))
	}
	if (
...
export function getActiveVaultEntry(
	registry: VaultRegistry
	...
	return (
		registry.entries.find(
			(entry) => entry.vaultInstanceId === runtimeActiveVaultInstanceId
		) ?? null
	)
}

export function getSelectedVaultEntryForReopen(
	registry: VaultRegistry
): VaultRegistryEntry | null {
	if (registry.activeVaultInstanceId === null) return null
	return (
		registry.entries.find(
			(entry) => entry.vaultInstanceId === registry.activeVaultInstanceId
		) ?? null
	)
}

export function parseVaultRegistry(
	value: unknown
): Result<VaultRegistry, LenaError> {
	...
	const invariant = parsed.error.issues.find(
