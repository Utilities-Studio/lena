...
import { parseEffectId, parseIsoTimestamp } from '@lena-inc/core'
import {
...
describe('Google Drive transport protocol', () => {
	test('binds immutable upload to runtime-verified local ciphertext', () => {
		...
		const fixture = verificationFixtures(generation)
		const attempt = activeUploadAttempt(
			generation,
			fixture.local,
			fixture.claimId
		)
		...
		expect(isAuthorizedGoogleDriveImmutableUploadPlan({ ...plan.value })).toBe(
			false
		)
		expect(
		...
		const persistedAttempt = parseBackupAttempt(
	...
	test('resolves conflict only for the exact claimed provider object', () => {
		...
		expect(
			reconcileGoogleDriveUploadConflict(plan, target, fixture.receipt).isOk()
		).toBe(true)
		expect(
			reconcileGoogleDriveUploadConflict(
				{ ...plan },
	...
	test('downloads and resumes only bounded exact operations', () => {
		...
		const fixture = verificationFixtures(generation)
		expect(
			createGoogleDriveListPlan(generation.vaultId, { pageSize: 250 }).isOk()
		).toBe(true)
		expect(
			createGoogleDriveListPlan(generation.vaultId, { pageSize: 0 }).isOk()
		).toBe(false)
		...
		if (download.isOk())
			expect(isAuthorizedGoogleDriveDeletePlan(download.value)).toBe(false)
		...
		expect(
			createGoogleDriveResumeUploadPlan(
				upload,
				checkpoint(upload, { remotePath: 'wrong' })
			).isOk()
		).toBe(false)
		expect(
			createGoogleDriveResumeUploadPlan(
	...
	test('does not resume a checkpoint from another upload', () => {
		...
		expect(
			createGoogleDriveResumeUploadPlan(second, checkpoint(first)).isOk()
		).toBe(false)
	})
	...
	test('deletes only with runtime verification and opaque retention authority', () => {
		...
		const deletion = createGoogleDriveDeletePlan(
			generation,
			fixture.remote,
			retention.value
		)
		...
		expect(
			createGoogleDriveDeletePlan(generation, fixture.remote, {
				...retention.value
			}).isOk()
		).toBe(false)
	})
	...
	test('keeps token and rate failures retryable and unknown codes fatal', () => {
		...
		expect(classifyUnknownGoogleDriveFailure('future-provider-code').kind).toBe(
			'fatal'
		)
	})
})
