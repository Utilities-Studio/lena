...
export function validateEmbeddingOutput(
	...
	if (!Array.isArray(value) && !(value instanceof Float32Array)) {
		return err(
			new LenaError('invalid_input', { boundary: 'ai_embedding_output' })
		)
	}
	const parsedEmbedding = z
		.array(z.number().finite())
		.safeParse(Array.from(value))
	...
	if (specification.normalized) {
		const magnitude = Math.sqrt(
			sumBy(embedding, (component) => component * component)
		)
		if (
			...
			return err(
				new LenaError('integrity_failed', { boundary: 'ai_embedding_output' })
			)
		}
	}
...
export function createDerivedEmbeddingRecord(
	...
	const embedding = validateEmbeddingOutput(
		input.embedding,
