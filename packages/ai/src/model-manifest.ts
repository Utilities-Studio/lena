...
export const modelManifestSchema = z
  ...
  .superRefine((manifest, context) => {
    if (manifest.requirements.minimumFreeStorageBytes < manifest.artifact.byteLength) {
      context.addIssue({
        code: "custom",
    ...
    if (manifest.capabilities.includes("embedding") !== (manifest.embedding !== null)) {
      context.addIssue({
        code: "custom",
...
export type ModelArtifactManifest = Readonly<z.infer<typeof modelArtifactManifestSchema>>;
export type ModelEmbeddingManifest = Readonly<z.infer<typeof modelEmbeddingManifestSchema>>;
export type ModelDeviceRequirements = Readonly<z.infer<typeof modelDeviceRequirementsSchema>>;
export type ModelManifest = Readonly<z.infer<typeof modelManifestSchema>>;

export function parseModelManifest(value: unknown): Result<ModelManifest, LenaError> {
