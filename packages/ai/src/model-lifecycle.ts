...
export interface ModelAvailability {
  ...
  readonly reason: "available" | "capability_missing" | "model_absent" | "model_not_ready";
}
...
const modelLifecycleEventSchema = z.discriminatedUnion("type", [
  ...
  z.strictObject({ ...lifecycleInstallIdEventFields, type: z.literal("download_completed") }),
  z.strictObject({
  ...
  z.strictObject({ ...lifecycleInstallIdEventFields, type: z.literal("retry_requested") }),
  z.strictObject({ ...lifecycleInstallIdEventFields, type: z.literal("eviction_requested") }),
  z.strictObject({ ...lifecycleInstallIdEventFields, type: z.literal("eviction_completed") }),
  z.strictObject({ ...lifecycleInstallIdEventFields, type: z.literal("artifact_discarded") }),
]);

export type ModelLifecycleEvent = Readonly<z.infer<typeof modelLifecycleEventSchema>>;
...
export function transitionModelLifecycle(
  ...
  return match(parsedEvent.data)
    ...
    .with({ type: "download_progressed" }, (event) => {
      if (state.status !== "downloading") return invalidTransition(state, event.type);
    ...
    .with({ type: "verification_completed" }, (event) => {
      if (state.status !== "verifying") return invalidTransition(state, event.type);
      const matching = requireInstallId(state.installId, event.installId);
      if (matching.isErr()) return err(matching.error);
    ...
    .with({ type: "operation_failed" }, (event) => {
      if (state.status === "absent" || state.status === "ready" || state.status === "failed") {
    ...
    .with({ type: "eviction_completed" }, (event) => {
      if (state.status !== "evicting") return invalidTransition(state, event.type);
      const matching = requireInstallId(state.installId, event.installId);
      if (matching.isErr()) return err(matching.error);
