...
export function chunkText(
	input: unknown
): Result<readonly TextChunk[], LenaError> {
	...
	if (codePoints.length > MAX_SOURCE_CODE_POINTS) {
		return err(
			new LenaError('limit_exceeded', { maximum: MAX_SOURCE_CODE_POINTS })
		)
	}
	...
	while (startCodePoint < codePoints.length) {
		const endCodePoint = Math.min(
			startCodePoint + maximumCodePoints,
			codePoints.length
		)
